...
class PugdagdMultiClient(object):
    ...
    async def request(self, command, params=None, timeout=60):
        try:
            return await self.__get_pugdagd().request(command, params, timeout=timeout, retry=1)
        except PugdagdCommunicationError:
            await self.initialize_all()
            return await self.__get_pugdagd().request(command, params, timeout=timeout, retry=3)
