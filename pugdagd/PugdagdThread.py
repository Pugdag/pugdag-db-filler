...
from .messages_pb2 import PugdagdMessage

MAX_MESSAGE_LENGTH = 1024 * 1024 * 1024  # 1GB


class PugdagdCommunicationError(Exception): pass


# pipenv run python -m grpc_tools.protoc -I./protos --python_out=. --grpc_python_out=. ./protos/rpc.proto ./protos/messages.proto ./protos/p2p.proto

class PugdagdThread(object):
    def __init__(self, pugdagd_host, pugdagd_port, async_thread=True):

        self.pugdagd_host = pugdagd_host
        self.pugdagd_port = pugdagd_port

        if async_thread:
            self.channel = grpc.aio.insecure_channel(f'{pugdagd_host}:{pugdagd_port}',
            ...
            self.channel = grpc.insecure_channel(f'{pugdagd_host}:{pugdagd_port}',
                                                 compression=grpc.Compression.Gzip,
                                                 options=[
                                                     ('grpc.max_send_message_length', MAX_MESSAGE_LENGTH),
                                                     ('grpc.max_receive_message_length', MAX_MESSAGE_LENGTH),
    ...
    async def request(self, command, params=None, wait_for_response=True, timeout=5):
        if wait_for_response:
            try:
                ...
                raise PugdagdCommunicationError(str(e))
